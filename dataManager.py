...
class dataManager:
    ...
    #         raise type(e)(
    #             f"Error in \033[94m {self.__class__.__name__}\033[0m, createReport function: \033[91m{str(e)}\033[0m")
    ...
    def downloadImages(self):
        try:
            if not self.overwrite:
                ...
                while os.path.exists(os.path.join(os.getcwd(), 'data', f"{i}_{datetime.date.today()}.json")):
                    i += 1
                with open(os.path.join(os.getcwd(), 'data', f"{i-1}_{datetime.date.today()}.json"), 'r') as file_save:
                    json_file = file_save
                    ...
                    for show in json_file:
                        response = requests.get(json_file[show]['image_link'], stream=True)
                        response.raise_for_status()
                        image_path = os.path.join(os.getcwd(), 'data', 'images', f"{show}.jpg")
